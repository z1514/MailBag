...
var Worker = /** @class */ (function () {
    ...
    Worker.prototype.updateContact = function (inContact) {
        ...
        return new Promise(function (inResolve, inReject) {
            _this.db.update({ _id: inContact._id }, { name: inContact.name,
                email: inContact.email }, {}, function (inError, inNumberUpdated) {
                if (inError) {
                    console.log("Contacts.Worker.updateContact(): Error", inError);
                    inReject(inError);
                }
                else {
                    console.log("Contacts.Worker.updateContact(): Ok", inNumberUpdated);
                    inResolve();
                }
            });
        });
